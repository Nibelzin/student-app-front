import { generateAcronym, getContrastColor } from '@/lib/utils'
    ...
    return (
        <a className='w-full md:w-52 h-30 md:h-52 flex justify-center items-center bg-white border rounded-md cursor-pointer hover:shadow-md transition-shadow' href={`/subject/${subject?.id}`}>
            <div className='p-4 md:p-2 flex md:flex-col items-center w-full gap-4'>
                <div className="w-20 h-20 rounded-full flex items-center justify-center font-semibold" style={{ backgroundColor: subject?.color ?? '#6b7280', color: subject?.color ? getContrastColor(subject.color) : 'white' }}>{acronym}</div>
